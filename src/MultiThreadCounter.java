...
// public class MultiThreadCounter {
//     private int totalCount = 0; // Shared cumulative count
//     private final ReentrantLock lock = new ReentrantLock(); // Lock for thread-safe updates

//     public int countWith4Threads(String rootPath, BlockingQueue<String> resultsQueue) {
//         File rootDir = new File(rootPath);
//         if (!rootDir.exists() || !rootDir.isDirectory()) {
//             throw new IllegalArgumentException("Invalid directory: " + rootPath);
//         }
...
//             Thread thread = new Thread(() -> {
//                 for (int j = start; j < end; j++) {
//                     countItems(topLevelItems[j], resultsQueue); // Pass resultsQueue
//                 }
//             });
...
//     private void countItems(File item, BlockingQueue<String> resultsQueue) {
//         if (item.isFile()) {
//             if (item.getName().toLowerCase().endsWith(".pdf")) {
//                 lock.lock(); // Lock before updating the shared count
//                 try {
//                     totalCount++; // Increment the shared cumulative count
//                     resultsQueue.put("[4 Threads] Found " + totalCount + " PDF in " + System.currentTimeMillis() + " ms");
//                 } catch (InterruptedException e) {
//                     Thread.currentThread().interrupt();
//                 } finally {
//                     lock.unlock(); // Unlock after updating
...
//         for (File file : files) {
//             countItems(file, resultsQueue); // Pass resultsQueue recursively
//         }
//     }
// }